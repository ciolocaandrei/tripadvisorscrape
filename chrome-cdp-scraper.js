/**
 * Chrome CDP Scraper - Uses your REAL Chrome browser with existing session
 *
 * SETUP:
 * 1. Open Chrome manually
 * 2. Go to chrome://version/
 * 3. Copy the "Profile Path"
 * 4. Visit TripAdvisor and solve CAPTCHA once in that browser
 * 5. Run this script - it will use your existing Chrome session
 */
...
const CONFIG = {
  ...
  DELAY_BETWEEN_HOTELS: 8000,
  DELAY_BETWEEN_PAGES: 4000,
  RESULTS_DIR: './results',

  // Update this with your Chrome profile path
  // Find it at chrome://version/ under "Profile Path"
  CHROME_USER_DATA: '/Users/ciolocaandrei/Library/Application Support/Google/Chrome',
...
async function extractHotelList(page) {
  ...
  if (bodyText.includes('unusual activity') || bodyText.includes('blocked')) {
    console.log('\n⚠️  STILL BLOCKED! You need to:');
    console.log('1. Open Chrome manually');
    console.log('2. Visit TripAdvisor');
    console.log('3. Solve CAPTCHA and browse normally');
    console.log('4. Keep Chrome open');
    console.log('5. Run this script again\n');
    throw new Error('Site blocked - manual intervention needed');
  }
...
async function main() {
  console.log('╔════════════════════════════════════════════════════════╗');
  console.log('║  TripAdvisor Scraper - Using Your Real Chrome Browser ║');
  console.log('╚════════════════════════════════════════════════════════╝\n');

  console.log('📋 This will use your existing Chrome session');
  console.log('   Profile: ' + CONFIG.CHROME_USER_DATA);
  console.log('   User: ' + CONFIG.CHROME_PROFILE);
  console.log('\n⚠️  Make sure you:');
  console.log('   1. Have visited TripAdvisor in Chrome');
  console.log('   2. Solved any CAPTCHAs');
  console.log('   3. Have cookies/session active\n');
  ...
  try {
    ...
    browser = await chromium.launchPersistentContext(
      `${CONFIG.CHROME_USER_DATA}/${CONFIG.CHROME_PROFILE}`,
      {
        headless: false,
    ...
    console.error('\n❌ Error:', error.message);
    if (error.message.includes('Target closed') || error.message.includes('blocked')) {
      console.log('\n💡 TIP: Try these steps:');
      console.log('   1. Open Chrome manually');
      console.log('   2. Visit TripAdvisor and browse normally');
