/**
 * Batch Runner for Review Extraction
 *
 * This script runs extract-reviews.js in batches to work within
 * Bright Data Scraping Browser page navigation limits.
 *
 * Usage: node run-review-batches.js
 */
...
const CONFIG = {
  TOTAL_HOTELS: 45,  // Adjust based on hotels-list.json
  BATCH_SIZE: 5,
...
function runBatch(batchStart) {
  return new Promise((resolve, reject) => {
    ...
    const process = spawn('node', ['extract-reviews.js'], {
...
async function main() {
  console.log('╔════════════════════════════════════════════════════════╗');
  console.log('║     Batch Runner - Review Extraction                  ║');
  console.log('╚════════════════════════════════════════════════════════╝\n');

  // Read actual hotel count from hotels-list.json
  const hotelsFile = './results/hotels-list.json';
  if (fs.existsSync(hotelsFile)) {
    const hotels = JSON.parse(fs.readFileSync(hotelsFile, 'utf8'));
    CONFIG.TOTAL_HOTELS = hotels.length;
  }
  ...
  console.log('\n' + '═'.repeat(60));
  console.log('🎉 ALL REVIEW BATCHES COMPLETED!');
  console.log('═'.repeat(60));
  ...
  const resultsPath = './results/reviews-summary.json';
